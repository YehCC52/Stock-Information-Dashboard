...
def test_fetch_overnight_premarket_skips_non_us_tickers(monkeypatch) -> None:
    requested: list[str] = []
    data = {
        "ES=F": {"regularMarketPrice": 5000.0, "regularMarketPreviousClose": 4950.0},
        "NQ=F": {"regularMarketPrice": 18000.0, "regularMarketPreviousClose": 18100.0},
    ...
    snapshot = fetch_overnight_premarket([
        TickerConfig(symbol="NVDA", company_name="NVIDIA"),
        TickerConfig(symbol="2330.TW", company_name="台積電", market="twse", currency="TWD"),
        TickerConfig(symbol="BTC-USD", company_name="Bitcoin", market="crypto"),
    ])

    assert "NVDA" in requested
    assert "2330.TW" not in requested
    assert "BTC-USD" not in requested
    assert [move.symbol for move in snapshot.watchlist_movers] == ["NVDA"]
